...
CREATE TABLE `user` (
    ...
    `balance`     DECIMAL(12,2) NOT NULL DEFAULT 10000.00 COMMENT '账户余额',
    `vip_level`   TINYINT      NOT NULL DEFAULT 0 COMMENT 'VIP等级: 0-普通 1~5',
    `preference`  VARCHAR(255) DEFAULT NULL COMMENT '购物偏好(逗号分隔的分类ID)',
    `auto_accept_order` TINYINT NOT NULL DEFAULT 0 COMMENT '是否自动接单: 0-否 1-是',
...
CREATE TABLE `product` (
    ...
    `image`       LONGTEXT DEFAULT NULL COMMENT '商品图片(base64或URL)',
